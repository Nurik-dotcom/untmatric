...
func start_level(level_idx):
	...
	for child in container_switches.get_children():
		...
		child.modulate = Color.WHITE # Reset Safe Mode highlight
...
func update_preview_display():
	# Visible only for Complexity A (Levels 0-14)
...
func _on_stability_changed(new_val, change):
	...
	if new_val <= 0:
		...
		btn_check.disabled = true # Lock check button in safe mode? Assuming yes.
