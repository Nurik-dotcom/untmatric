...
func _on_system_failure():
	...
	game_over_label.text = "CRITICAL LOGIC FAILURE\nSYSTEM HALTED"
	input_a_btn.disabled = true
	input_b_btn.disabled = true
	gate_selector.disabled = true
	btn_verdict.disabled = true

	# Glitch visual effect simulation
	var tween = create_tween()
	tween.tween_property($MainLayout, "modulate", Color(1, 0, 0), 0.1)
	tween.tween_property($MainLayout, "modulate", Color(1, 1, 1), 0.1)
	tween.set_loops(5)
