...
func set_ready() -> void:
	_apply_state("МАСКА ГОТОВА", Color(0.95, 0.88, 0.55, 1.0), Color(0.18, 0.16, 0.08, 0.9))

func set_applied() -> void:
	_apply_state("И ПРИМЕНЯЕТСЯ", Color(0.62, 0.95, 1.0, 1.0), Color(0.08, 0.14, 0.18, 0.9))

func set_open() -> void:
	_apply_state("ЗАМОК ОТКРЫТ", Color(0.4, 1.0, 0.6, 1.0), Color(0.08, 0.18, 0.1, 0.9))

func set_error() -> void:
	_apply_state("ОШИБКА БЛОКИРОВКИ", Color(1.0, 0.4, 0.4, 1.0), Color(0.22, 0.05, 0.05, 0.9))
