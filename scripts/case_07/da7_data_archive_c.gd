...
func _refresh_case_ui_i18n() -> void:
	...
	title_label.text = _tr("da7.c.ui.title_running", "CASE #7: SQL MASTER [{case_id}]", {"case_id": case_id})
	if not _status_i18n_key.is_empty():
		status_label.text = _tr(_status_i18n_key, _status_i18n_default, _status_i18n_params)
...
func _render_case() -> void:
	...
	prompt_label.text = "[b]%s[/b]" % str(current_case.get("prompt", _tr("da7.c.ui.default_prompt", "Assemble the SQL sequence.")))
	...
	_set_status_i18n("da7.c.ui.status_initial", "Assemble the query tokens and submit.")
	_update_timer_ui()
	_update_stability_ui()
